...
Page({
  ...
  select_date(value) {       //时间选择
    ...
    if (value.detail == 0) {
      ...
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7); // 得到7天前的日期
      var list_items = list_items.filter(item => {
        var itemDate = new Date(item.createDate);
      ...
      monthDaysAgo.setDate(monthDaysAgo.getDate() - 30); // 得到30天前的日期
      var list_items = list_items.filter(item => {
        var itemDate = new Date(item.createDate);
      ...
      console.log('===list_items', list_items)
      list_items.sort((a, b) => new Date(a.createDate).getTime() - new Date(b.createDate).getTime());
      console.log('===list_items', list_items)
    }
  ...
  onViewClick(event) {
    ...
    wx.navigateTo({
      url: '/pages/item-detail/item-detail?id=' + id + '&manner=' + "list" + '&type=' + event.currentTarget.dataset.type,
    });
  },
  onEditClick(event) {
    const id = event.currentTarget.dataset.id;
  ...
  onSelect(event) {
    ...
    var that = this
    var token = wx.getStorageSync('cookie').split('token=')[1].split(';')[0]
    if (this.data.shareVideo) {
      wx.request({
        url: "http://127.0.0.1:8888/my/share",
        data: { token: token, videoid: that.data.share_item, can_userid: event.detail.userid },
