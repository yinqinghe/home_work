...
Page({
    ...
    data: {
        ...
        optionsAnimal,
        pictureShow: false,
        currentDate: 1259546574,    //设定选择的默认日期2010-0-00
        minDate: 946692174000,         //设定日期最小日期2000-0-00
    ...
    confirm(event) {
        ...
        console.log('===onInpute', this.data.currentDate)
        const str = this.data.currentDate
        const dateObj = new Date(str);
        const year1 = dateObj.getFullYear();
        const month = ("0" + (dateObj.getMonth() + 1)).slice(-2);
        const day = ("0" + dateObj.getDate()).slice(-2);

        const formatDate = `${year1}-${month}-${day}`;
        console.log('===onInpute', formatDate)
        this.setData({ birthday: formatDate, });


        this.setData({ showDate: false });
        const year = Number(this.data.birthday.split('-')[0])
        console.log('===year', year % 12)
        console.log('===year', typeof year)

        // 计算生肖
        var animals = ['申猴', '酉鸡', '戌狗', '亥猪', '子鼠', '丑牛', '寅虎', '卯兔', '辰龙', '巳蛇', '午马', '未羊'];
