...
Page({
    ...
    onViewClick(event) {
        ...
        wx.navigateTo({
            url: '/pages/item-detail/item-detail?id=' + id + '&item=' + tom + '&share=true' + '&type=' + type,
        });
    },
    cancelShare(event) {
        const id = event.currentTarget.dataset.id;
        // console.log('===', id)
        wx.request({
            url: "http://127.0.0.1:8888/my/friend/del_share?item_id=" + id,
            method: "GET",
    ...
    show_btn(event) {
        ...
        for (var i in list_items) {
            console.log('Name:', list_items[i].videoid_d !== null);
            if (list_items[i].videoid_d !== null) {
                list_items[i].videoid_d[0].video_url = 'http://127.0.0.1:8888/' + list_items[i].videoid_d[0].video_url
                list_items[i].type = '视频'
            } else {
                if (list_items[i].workid_d[0].image_url) {
                    list_items[i].workid_d[0].image_url = 'http://127.0.0.1:8888/' + list_items[i].workid_d[0].image_url
    ...
    onLoad(options) {
        ...
        var that = this;
        wx.request({
            url: "http://127.0.0.1:8888/my/friend/myshare?token=" + token,
            success: function (res) {
